    ...
    <h-container>
      <h-header height="auto">
        ...
        >
          ...
          <h-form-item label="管教审批时间" prop="valueDatetwo">
            <h-date-picker
          ...
          <h-form-item label="大队领导审批时间" prop="valueDatethree">
            <h-date-picker
              v-model="valueDatethree"
              type="daterange"
              range-separator="至"
              start-placeholder="开始日期"
              end-placeholder="结束日期"
          ...
          <!-- valueDatefour -->
          <h-form-item label="发货时间" prop="valueDatefour">
            <h-date-picker
              v-model="valueDatefour"
              type="daterange"
              range-separator="至"
              start-placeholder="开始日期"
              end-placeholder="结束日期"
          ...
          <h-form-item>
            ...
            <h-dropdown>
              ...
              <template #dropdown>
                <h-dropdown-menu>
                  <h-dropdown-item @click="orderApproval"
                    >审批已选择订单</h-dropdown-item
                  >
                  <h-dropdown-item @click="wardApproval"
                    >按病室审批</h-dropdown-item
                  >
                  <!-- <h-dropdown-item>按管教审批</h-dropdown-item> -->
                </h-dropdown-menu>
              </template>
            </h-dropdown>
            <!--  -->
  ...
  setup() {
    ...
    const state = reactive<IState>({
      ...
      formInline: {
        ...
        gjspsjks: '',
        gjspsjjs: '',
        ddldspsjjs: '', // (string, optional): 大队领导审批时间结束 ,
        ddldspsjks: '',
        ffsjjs: '', // (string, optional): 发货时间结束 ,
        ffsjks: '',
      },
    ...
    const batchReceiving = (index:number, row:IList) => {
      ...
      if (state.multipleSelection.length > 0 && status.length === 0) {
        ...
        HMessage({
          type: 'success',
          message: '请先选择要收货的列表! 或您的选择中包含已发货选项'
        })
      }
      // const id = state.multipleSelection
      // const idArr:any[] = []
    ...
    const onSubmit = () => {
      ...
      if (state.valueDatetwo.length !== 0) {
        const start:string = moment(state.valueDatetwo[0]).format('YYYY-MM-DD HH:mm:ss')
        const end:string = moment(state.valueDatetwo[0]).format('YYYY-MM-DD HH:mm:ss')
        state.formInline.gjspsjks = start
        state.formInline.gjspsjjs = end
      }
      // 大队领导
      if (state.valueDatethree.length !== 0) {
        const start:string = moment(state.valueDatethree[0]).format('YYYY-MM-DD HH:mm:ss')
        const end:string = moment(state.valueDatethree[0]).format('YYYY-MM-DD HH:mm:ss')
        state.formInline.ddldspsjks = start
        state.formInline.ddldspsjjs = end
      }
      // 发货时间valueDatefour
      if (state.valueDatefour.length !== 0) {
        const start:string = moment(state.valueDatethree[0]).format('YYYY-MM-DD HH:mm:ss')
        const end:string = moment(state.valueDatethree[0]).format('YYYY-MM-DD HH:mm:ss')
        state.formInline.ffsjks = start
        state.formInline.ffsjjs = end
      }
    ...
    const onReset = (formName: string): void => {
      state.valueDate = ''
      state.valueDatetwo = ''
      state.valueDatethree = ''
      state.valueDatefour = ''
      state.formInline.xfsjks = ''
      state.formInline.xfsjjs = ''
      state.formInline.gjspsjks = ''
      state.formInline.gjspsjjs = ''
      state.formInline.ddldspsjks = ''
      state.formInline.ddldspsjjs = ''
      state.formInline.ffsjks = ''
      state.formInline.ffsjjs = ''
    ...
    const www = (newArr:any, is:string) => {
      newArr[is].forEach((item:any) => {
